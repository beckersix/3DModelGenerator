...
class LearningCommandInterpreter:
    ...
    def interpret_command(self, text):
        ...
        # Add adaptive generation commands
        if "generate variations" in text.lower():
            return {
                'command_type': 'generate',
                'subtype': 'variations',
                'params': self._extract_variation_params(text)
            }
        elif "train adaptive" in text.lower():
            return {
                'command_type': 'train',
                'subtype': 'adaptive',
                'params': self._extract_training_params(text)
            }
    ...
    def _extract_variation_params(self, text):
        """Extract parameters for variation generation from text"""
        params = {}
        
        # Try to find number of variations
        num_match = re.search(r'(\d+)\s*variations?', text)
        if num_match:
            params['num_variations'] = int(num_match.group(1))
        else:
            params['num_variations'] = 3  # default
            
        # Try to find input model path
        path_match = re.search(r'from\s+["\']?([^"\']+\.(?:obj|stl|ply))["\']?', text)
        if path_match:
            params['input_model'] = path_match.group(1)
            
        return params
        
    def _extract_training_params(self, text):
        """Extract parameters for adaptive training from text"""
        params = {}
        
        # Try to find number of epochs
        epochs_match = re.search(r'(\d+)\s*epochs?', text)
        if epochs_match:
            params['num_epochs'] = int(epochs_match.group(1))
            
        # Try to find batch size
        batch_match = re.search(r'batch\s*size\s*[=:]\s*(\d+)', text)
        if batch_match:
            params['batch_size'] = int(batch_match.group(1))
            
        # Try to find model directory
        dir_match = re.search(r'(?:in|from|to)\s+directory\s+["\']?([^"\']+)["\']?', text)
        if dir_match:
            params['model_dir'] = dir_match.group(1)
